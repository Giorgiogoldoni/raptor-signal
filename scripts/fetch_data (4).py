...
def main():
    ...
    # ── BACKFILL stoxx_eem da stoxx_spy / eem_spy ────────────
    # Calcola stoxx_eem per tutti i record storici che non ce l'hanno
    # stoxx_eem = stoxx_spy / eem_spy (per transitività del triangolo)
    backfilled = 0
    for rec in history:
        if rec.get("stoxx_eem") is None:
            sp = rec.get("stoxx_spy")
            ep = rec.get("eem_spy")
            if sp and ep and ep != 0:
                rec["stoxx_eem"] = round(sp / ep, 6)
                backfilled += 1
    if backfilled > 0:
        print(f"  ✓ Backfill stoxx_eem: {backfilled} record aggiornati")
